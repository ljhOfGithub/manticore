...
class Testcase:
    """
    A `WorkspaceTestCase` represents a test case input generated by Manticore.“WorkspaceTestCase”表示由Manticore生成的测试用例输入。
    """
...
class Store:
    ...
    # can choose the pair it's best optimized for.Save_value /load_value和save_stream/load_stream是相互实现的。后备存储可以选择最适合它的一对。
...
class FilesystemStore(Store):
    ...
    def stream(self, key: str, mode: str = "r", lock: bool = False):
        """
        Yield a file object representing `key`生成一个表示“key”的文件对象
...
class Workspace:
    """
    A workspace maintains a list of states to run and assigns them IDs.工作区维护要运行的状态列表，并为它们分配id。
    """
    ...
    def load_state(self, state_id, delete=True):
        """
        Load a state from storage identified by `state_id`.从存储中加载一个由' state_id '标识的状态。
