...
class Testcase:
    ...
    “WorkspaceTestCase”表示由Manticore生成的测试用例输入。
    """
...
class Store:
    ...
    “Store”可以保存任意键/值(包括状态)和文件流。用于产生输出、保存状态和加载状态。
    In subclasses:
    ...
    *这被用作存储描述符的前缀
    ...
    # Save_value /load_value和save_stream/load_stream是相互实现的。后备存储可以选择最适合它的一对。
    ...
    @contextmanager
    def save_stream(self, key: str, binary: bool = False):#作为上下文管理器类
        """
        Return a managed file-like object into which the calling code can write
        arbitrary data.
        返回一个托管文件类对象，调用代码可以向该对象写入任意数据。
        :param key:
    ...
    def load_stream(self, key: str, binary: bool = False):
        ...
        返回一个托管文件类对象，调用代码可以从中读取以前序列化的数据。
        :param key:
        :return: A managed stream-like object
        """
    ...
    def save_state(self, state: StateBase, key: str) -> None:
        """
        Save a state to storage.将状态保存到存储中。
    ...
    def load_state(self, key: str, delete: bool = True) -> StateBase:
        """
        Load a state from storage.从存储器加载一个状态。
...
class ManticoreOutput:
    ...
    def descriptor(self):
        ...
        返回创建此工作区的描述符。描述符的格式为<type>:<uri>，其中type表示介质。</uri></type>
        :rtype: str
        """
    ...
    def _named_stream(self, name, binary=False, lock=False):
        ...
        创建一个索引输出流，例如。“test_00000001.name”
    ...
    def save_testcase(self, state: StateBase, testcase: Testcase, message: str = ""):
        ...
        将环境从“状态”保存到存储中。返回一个描述它的状态id，它应该是一个整型或字符串。
